/*
	Automaton representa um indivíduo do algoritmo genético, ele extende o personagem, adicionando características como o seu genoma e a capacidade de movimentos automatizados
*/
...
class Automaton extends Char{
	...
	move(){
		...
		if(this.stats == "playing" && (this.atMoveIndex < this.moveset.length)){
			...
			if(debug)
				console.log("GAME AutomatonClass: Movendo para: " + movement + " -" + this.atMoveIndex + " de " + this.moveset.length);
			switch(movement){
				case "up":
					this.up(roadHeight);
					break;
				...
				case "right":
					...
					break;
				default:
					console.log("ERROR AutomatonClass: Direção no moveset de automato ID: " + this.id + " inválida. Direção: " +this.atMoveIndex +"-" + movement);
					alert("ERROR: Direção no moveset de automato ID: " + this.id + " inválida:" + movement);
