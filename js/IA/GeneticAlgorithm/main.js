    ...
    Ticker.start(delay, function() {
        ...
        if (countTester >= totalMoves) {
            ...
            for (var i = 0; i < population.length; i++) {
                if (population[i].stats == "winner") completed++;
...
function crossElements(el1, el2) {
    ...
    for (var i = 0; i < maxMoves; i++) {
        ...
        if (random < 500) {
            if (debug) console.log("I = " + i + " Herdou " + direction + " de + ID:" + el1.id);

            var mut = mutation();
            if (mut == false) direction = el1.moveset[i];
            else {
                console.log("Mutação!!");
                direction = mut;
            }
        } else {
            if (debug) console.log("I = " + i + "Herdou " + direction + " de + ID:" + el2.id);
            var mut = mutation();
            if (mut == false) direction = el2.moveset[i];
            else {
                console.log("Mutação!!");
                direction = mut;
            }
...
function testElement(e) {
    ...
    if (debug) console.log("Pontuação personagem ID:" + e.id + ": " + e.pontuation);
...
function getRandDir() {
    ...
    switch (directionNumber) {
        ...
        default:
            console.log(
                "ERRO: O Número sorteado não é válido, por algum motivo... Numero: " +
                    directionNumber
            );
...
/* Algoritmo para ordenar a população, código do insertionSort 
   copiado e adaptado(para ordenar de trás para frente) de: http://codingmiles.com/sorting-algorithms-insertion-sort-using-javascript/
*/
