...
var charPlayingSprite = {"src":"img/chk1.png","up" : {"x":0, "y":144, "s":48}, "right" : {"x":0, "y":96, "s":48}, "left" : {"x":0, "y":48, "s":48}, "down" : {"x":0, "y":0, "s":48} };
...
function init(){
	...
	characters.push(new Char(320,roadHeight*roads.length+startY, roadHeight, roadHeight, "green", characters.length));
...
function initCars(){
	...
	for(var i = 0; i < roads.length; i++){
		for(var j = 0; j < carsPerRoad; j++){
			...
			if(debug)
				console.log("RAND: posicao " + pos + " da via " + i);
			
			while( !roads[i].addCar(pos*carWidth, carColor) ){
				pos = Math.floor(Math.random() * (carsCells + 1));
				if(debug)
					console.log("RAND: RE-Sorteio: posicao " + pos + " da via " + i);
			}
			//updateRoads();
		}
	}
